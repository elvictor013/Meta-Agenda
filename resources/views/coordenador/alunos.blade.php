...
@section('page-subtitle','Gerencie os alunos das turmas')
...
<button onclick="openModal('modalCriar')" class="flex items-center gap-2 bg-brand-900 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-brand-800">
    <span class="ms">add</span> Novo Aluno
</button>
@endsection
...
<div class="pt-2">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-16 text-center shadow-sm">
        <span class="ms text-6xl text-slate-200 block mb-4">school</span>
        <p class="text-slate-500 font-semibold">Nenhum aluno cadastrado.</p>
    </div>
    @else
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    <th class="text-left px-6 py-3 font-semibold text-slate-600">Aluno</th>
                    <th class="text-left px-6 py-3 font-semibold text-slate-600">Matrícula</th>
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 font-bold text-sm">{{ strtoupper(substr($aluno->nome,0,1)) }}</div>
                            <span class="font-semibold text-slate-800">{{ $aluno->nome }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ $aluno->matricula }}</td>
                    <td class="px-6 py-4"><span class="px-2 py-0.5 bg-brand-50 text-brand-700 text-xs font-semibold rounded-full">{{ $aluno->turma->nome ?? '—' }}</span></td>
                    <td class="px-6 py-4 text-slate-500 text-xs">{{ $aluno->turma->curso->nome ?? '—' }}</td>
                    <td class="px-6 py-4 text-right">
                        <button onclick='openEdit(@json($aluno))' class="p-1.5 rounded-lg text-slate-400 hover:text-brand-600 hover:bg-brand-50"><span class="ms">edit</span></button>
                        <button onclick="confirmDelete('{{ route('coord.alunos.delete',$aluno->id) }}')" class="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50"><span class="ms">delete</span></button>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<div id="modalCriar" class="hidden fixed inset-0 z-50 modal-overlay bg-black/40 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-bold text-slate-800">Novo Aluno</h3>
            <button onclick="closeModal('modalCriar')" class="text-slate-400"><span class="ms">close</span></button>
        </div>
        <form method="POST" action="{{ route('coord.alunos.store') }}" class="px-6 py-4 space-y-4">
            @csrf
            <div><label class="block text-xs font-semibold text-slate-600 mb-1">Nome Completo</label>
                <input type="text" name="nome" required placeholder="Nome do aluno" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"></div>
            <div><label class="block text-xs font-semibold text-slate-600 mb-1">Matrícula</label>
                <input type="text" name="matricula" required placeholder="Ex: 2024001" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-500"></div>
            <div><label class="block text-xs font-semibold text-slate-600 mb-1">Turma</label>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 bg-brand-900 text-white px-4 py-2.5 rounded-xl text-sm font-semibold">Cadastrar</button>
            </div>
        </form>
    </div>
</div>
<div id="modalEditar" class="hidden fixed inset-0 z-50 modal-overlay bg-black/40 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-bold text-slate-800">Editar Aluno</h3>
        ...
        <form method="POST" id="formEditar" class="px-6 py-4 space-y-4">
            ...
            <div><label class="block text-xs font-semibold text-slate-600 mb-1">Matrícula</label>
                <input type="text" name="matricula" id="edit_matricula" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-500"></div>
            <div><label class="block text-xs font-semibold text-slate-600 mb-1">Turma</label>
                <select name="turma_id" id="edit_turma_id" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                    @foreach($turmas as $t)<option value="{{ $t->id }}">{{ $t->nome }} — {{ $t->curso->nome ?? '' }}</option>@endforeach
                </select></div>
            <div class="flex gap-3 pt-2">
...
<div id="modalDelete" class="hidden fixed inset-0 z-50 modal-overlay bg-black/40 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box p-6 text-center">
        ...
        <p class="text-sm text-slate-500 mb-6">Excluir este aluno do sistema?</p>
...
function openEdit(a){
    document.getElementById('formEditar').action = `/coordenador/alunos/${a.id}`;
    document.getElementById('edit_nome').value = a.nome;
    document.getElementById('edit_matricula').value = a.matricula;
    document.getElementById('edit_turma_id').value = a.turma_id;
