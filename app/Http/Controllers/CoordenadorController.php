...
class CoordenadorController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        return view('coordenador.dashboard', compact(
            ...
            'totalProfessores', 'alocacoesRecentes', 'notificacoesRecentes'
        ));
    }
    ...
    public function storeTurma(Request $request)
    {
        ...
        Turma::create($request->only('semestre', 'turno', 'curso_id'));
    ...
    public function updateTurma(Request $request, $id)
    {
        ...
        $turma->update($request->only('semestre', 'turno', 'curso_id'));
        return back()->with('success', 'Turma atualizada!');
    }

    public function deleteTurma($id)
    {
        Turma::whereIn('curso_id', $this->cursosIds())->findOrFail($id)->delete();
    ...
    public function storeAlocacao(Request $request)
    {
        ...
        $conflito = $this->verificarConflito(
            $request->professor_id, $request->sala_id,
            $request->turmas, $request->dia_semana,
            $request->hora_inicio, $request->hora_fim
        );
        if ($conflito) return back()->with('error', $conflito)->withInput();
    ...
    public function updateAlocacao(Request $request, $id)
    {
        ...
        $conflito = $this->verificarConflito(
            $request->professor_id, $request->sala_id,
            $request->turmas ?? [], $request->dia_semana,
            $request->hora_inicio, $request->hora_fim, $id
        );
        if ($conflito) return back()->with('error', $conflito);
    ...
    private function verificarConflito($professorId, $salaId, $turmasIds, $dia, $inicio, $fim, $ignorarId = null)
    {
        ...
        if (!empty($turmasIds) && $query->clone()->whereHas('turmas', fn($q) => $q->whereIn('turmas.id', $turmasIds))->exists())
